...
class RealAdapter:
    ...
    def __init__(self) -> None:
        ...
        self.window_to_panel: Dict[str, str] = {v: k for k, v in self.panel_to_window.items()}
    ...
    def list_groups(self) -> List[Group]:
        ...
        try:
            ...
            for hg in halio_groups:
                ...
                member_panel_ids = [
                    self.window_to_panel.get(wid, wid) 
                    for wid in member_window_ids
                ]
    ...
    def set_panel(self, panel_id: str, level: TintLevel, min_dwell: int) -> bool:
        ...
        try:
            ...
            if response.status_code == 202:
                logger.info(f"Tint command accepted for panel {panel_id} → level {level}")
                # Update cache
                self._state_cache[window_id] = {
                ...
                logger.error(f"Halio API error: {response.status_code} - {response.text}")
                return False
    ...
    def set_group(self, group_id: str, level: TintLevel, min_dwell: int) -> List[str]:
        ...
        try:
            ...
            if response.status_code == 202:
                ...
                for g in groups:
                    if g.id == group_id:
                        logger.info(f"Tint command accepted for group {group_id} → level {level}")
